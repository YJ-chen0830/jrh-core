...
`pb sc fw sh lf tb tm ro rb fn hyd lc sw bsm rcb rcd rcpm spl stc dwt dr adm mx prw prm gad asd csd stk sla spc buc ffa phl scu ssc tbm dwd esd tsp ssb ssbm mfv pss gsd fsh cna ssd sls bsg scd cct std tsb hdc wfc hjc beb ejm tfe bsl fpd hcd vcd llp mfd pdh evm cfs rme lcc tia mml csv fpa tel feq cra ram cdh`

（`ssb` = slope-stability／無限邊坡法，`ssbm` = slope-stability-bishop／簡化Bishop法圓弧分片法——這兩個原本都誤用 `ssb`，2026-07 已修正 bishop 版本為 `ssbm`，避免 localStorage 資料互相覆蓋、專案工作流打卡對不上。新工具請避開這兩個碼。）
