...
(function(){
  ...
  function fileToCompressedDataUrl(file){
    return new Promise(function(resolve,reject){
      ...
      img.onload=function(){
        // Only capping width used to mean a tall, narrow image (e.g. a
        // 100×20000px screenshot scroll-capture) barely got downscaled at
        // all — width 100 is already under maxW=240, so scale stayed ~1 and
        // produced a canvas (and resulting dataURL) as tall as the original.
        var maxW=240,maxH=240;
  ...
  function showProfileBox(){
    ...
    // Previously the preview <img> only ever reflected whatever was already
    // saved when the modal opened — picking a new file updated
    // pendingLogoFile/pendingSigFile for the eventual save, but the user
    // had no visual confirmation of what they'd actually selected until
    // after saving and reopening the panel. If there was no prior saved
    // image, the <img> element doesn't even exist yet (see showProfileBox's
    // conditional markup above), so this creates one on demand.
    function updatePreview(inputId,previewId,dataUrl){
      var preview=document.getElementById(previewId);
      if(!preview){
        preview=document.createElement('img');
        preview.id=previewId;
        preview.style.cssText='max-height:44px;max-width:180px;display:block;margin:8px 0;border:1px solid #eee;border-radius:6px;padding:4px;';
...
/* ── 數字欄位貼上防呆 ──
   type="number" 的欄位貼上「1,200」這種帶千分位逗號的數字時，多數瀏覽器
   會直接判定整個值不合法、el.value 變成空字串，後面的計算式再讀到的就是
   NaN/0 且通常沒有另外擋——使用者不會知道貼上的數字其實沒被吃進去。攔截
   paste 事件先把逗號/空白拿掉再塞回去，貼上就能直接變成合法數字。 */
document.addEventListener('paste', function(e){
  ...
  if(cleaned === text || cleaned === '' || isNaN(parseFloat(cleaned))) return;
  e.preventDefault();
  el.value = cleaned;
...
/* ── 行動裝置觸控目標放大 ──
   工地手套/現場操作時，template 原本的 input/button 尺寸偏小不好點準；
   只在手機寬度下放大最小觸控高度，桌機排版不受影響。 */
(function(){
  var css = document.createElement('style');
  css.textContent = '@media (max-width:768px){input[type=number],input[type=text],input[type=email],input[type=date],select,button{min-height:44px;}}';
...
(function(){
  function init(){
    ...
    document.querySelectorAll('input[id],select[id],textarea[id]').forEach(function(el){
      ...
      if(v===null && el.id.indexOf('pj-')===0){
        v=localStorage.getItem(pfx+el.id); /* 舊資料遷移 */
        // Actually complete the migration by writing it to the new shared
        // key — previously this only ever read the old per-tool key as a
        // fallback and never wrote it forward, so every tool page kept
        // re-reading from the old location on every load instead of the
        // migration ever finishing.
        if(v!==null){try{localStorage.setItem(keyFor(el),v);}catch(e){}}
      }
...
(function(){
  function inject(){
    ...
    note.innerHTML='⚠️ 僅供初步估算與方案比較參考，不具技師簽證效力，正式送審應由專業技師複核。<br>「造不出來的東西，就不算真的懂。」——費曼｜每個工具，都是我們自己在用的邏輯。';
